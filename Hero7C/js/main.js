...
function relocation(){
	...
	var elewid = 990;
	$("header").css("width",allwid);	
	if(allwid>elewid){		
		$("aside").css("left", (allwid-elewid)/2);
		$("article").css("left", (allwid-elewid)/2);
	}
	else{
		$("aside").css("left", 0);
		$("article").css("left",0);
...
function stopMusic(){
	...
	$("#play").css("display", "inline");
	$("#pause").css("display", "none");
	$("#audio").attr('src','media/' + global_tag + '.mp3');
...
//设定基本能力	
	...
	for (var i = 1; i <= 7 ; i++)
	{
		var isEX = ability[i-1];
		if(isEX === 'X') { isEX = "EX";}
		$('#ab'+i+'i').attr('src','img/icon/level'+isEX+'.png');
		$('#ab'+i).text(isEX);
	}
//宝具
